...
const TopGainers = ({ type }: { type: "complete" | "summary" }) => {
  const PageSize = 10;
  ...
  const currentTableData = useMemo(() => {
    ...
    return (gainersMock || []).slice(firstPageIndex, lastPageIndex);
  }, [currentPage, gainersMock]);
  return (
    <div className="rounded-xl group/item">
      <Card className="w-full rounded-xl p-4">
        <div className="flex justify-between items-center">
          <p className="text-amber-400 font-bold">Principales ganadores</p>
          {type === "summary" && (
            <Link
