...
const TopActives = ({ type }: { type: "complete" | "summary" }) => {
  const PageSize = 10;
  ...
  const currentTableData = useMemo(() => {
    ...
    return (activesMock || []).slice(firstPageIndex, lastPageIndex);
  }, [currentPage, activesMock]);
  return (
    <div className="rounded-xl group/item">
      <Card className="w-full rounded-xl p-4">
        <div className="flex justify-between items-center">
          <p className="text-amber-400 font-bold"> Más activos</p>
          {type === "summary" && (
            <Link
                    ...
                    <Card className="bg-zinc-950  shadow-slate-500 border-none">
                      <CardContent className="flex justify-between p-6">
                        ...
                        >
                          ...
                          <span
                            className={`text-xs font-semibold text-green-400 ml-2 ${
